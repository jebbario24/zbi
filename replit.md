...
- **Progressive Profile Completion System:** Streamlined driver onboarding with quick signup (email/password OR Google OAuth) followed by gradual profile completion in Settings. Drivers can sign up in seconds and complete their profile at their own pace across 5 organized tabs.
- **Driver Authentication:** Dual authentication system supporting both email/password (bcrypt hashed) AND Google OAuth at `/driver/signup` and `/driver/login`. Creates users with role='driver'. Separate from restaurant owner authentication to prevent role confusion. Session-based authentication validates driver role before granting access.
- **Profile Completion Tracking:** Automated system calculates completion based on 19 required fields across personal info (8 fields), vehicle details (6 fields), license info (2 fields), documents (2 URLs), and bank connection (Stripe account). System automatically recalculates and persists `profileComplete` flag after every profile update via `updateProfileCompletion()` helper function.
- **Driver Settings Interface:** Comprehensive 5-tab settings page for profile management: (1) Personal Info tab with phone, DOB, address, emergency contacts; (2) Vehicle Details tab with type, make, model, year, plate, color, license info; (3) Documents tab with file uploads for ID proof and insurance via Uppy; (4) Bank Account tab with Stripe Connect Express onboarding; (5) Profile Status tab showing completion progress and admin approval status.
- **Application Review Workflow:** Platform admin reviews pending driver applications after drivers complete their profiles. Admin can approve or reject applications via `/api/driver/applications/pending`. Only approved drivers (`adminApproved = true`) can access orders and start accepting deliveries.
- **Stripe Connect for Drivers:** Drivers connect bank accounts through Stripe Connect Express onboarding (matching restaurant implementation). Stripe handles KYC, tax forms, international banking in 40+ countries. Drivers can connect Stripe BEFORE admin approval to complete their profile. No sensitive banking data stored in database. Driver dashboard shows payout connection status and earnings.
- **Order Gating System:** Orders only visible and accessible when BOTH conditions are met: `profileComplete === true` (all 19 fields filled + Stripe connected) AND `adminApproved === true` (admin approved application). Dashboard displays appropriate messaging for each state: incomplete profile, pending approval, or ready to work.
