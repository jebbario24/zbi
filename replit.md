...
- **Comprehensive CRUD Operations:** Complete create, read, update, and delete functionality for menu items, reservations, and inventory, ensuring consistent UI patterns and optimistic UI updates with React Query cache invalidation. Menu items support duplication via Copy button (hover-revealed alongside Edit/Delete), which creates a new item with " (Copy)" appended to the name while preserving all other attributes (description, price, category, image, availability). Both duplicate and delete endpoints enforce restaurant ownership verification to prevent cross-tenant data access.
